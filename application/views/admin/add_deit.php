            ...
            <div class="container-fluid">
                <div class="row">
                    <div class="col-md-12 col-lg-12 col-sm-12">
                        <div class="white-box">
                            <?php if(!empty($diet_data)){ ?>
                            <form method="post" action="<?php echo base_url()."deitp_update"; ?>">
                            <?php }else{ ?>
                                <form method="post" action="<?php echo base_url()."deitp_add"; ?>">
                            <?php } ?>
                            <div class="d-md-flex mb-3">
                                <h3 class="box-title mb-0">Add Deit Plan Details</h3>
                            ...
                            <div class="row">
                                <div class="col-md-4">
                                    ...
                                    <select class="form-control" name="workout_id">
                                        ...
                                        <?php
                                        foreach ($workout_all as $key => $value) { ?>
                                            <option value="<?php echo $value->plan_id; ?>" <?php if(!empty($diet_data) && $diet_data[0]->workout_id ==$value->plan_id){ echo"selected"; } ?>><?php echo $value->plan_name; ?></option>
                                        <?php }
                                        ?>
                                    </select>
                                </div>
                                <div class="col-md-2">
                                    ...
                                    <input type="text" class="form-control" name="min_height" <?php if(!empty($diet_data)){ echo"value='".$diet_data[0]->min_height."'"; } ?>>
                                </div>
                                <div class="col-md-2">
                                    <label>Max Height</label>
                                    <input type="text" class="form-control" name="max_height" <?php if(!empty($diet_data)){ echo"value='".$diet_data[0]->max_height."'"; } ?>>
                                </div>
                                <div class="col-md-2">
                                    <label>min Weight</label>
                                    <input type="text" class="form-control" name="min_weight" <?php if(!empty($diet_data)){ echo"value='".$diet_data[0]->min_weight."'"; } ?>>
                                </div>
                                <div class="col-md-2">
                                    <label>max Weight</label>
                                    <input type="text" class="form-control" name="max_weight" <?php if(!empty($diet_data)){ echo"value='".$diet_data[0]->max_weight."'"; } ?>>
                                </div>
                            </div>
                            <br>
                            <?php 
                            if(!empty($diet_data)){
                            $get_foods_m = explode(",",$diet_data[0]->foods_m);
                            $get_foods_an = explode(",",$diet_data[0]->foods_an);
                            $get_foods_e = explode(",",$diet_data[0]->foods_e);
                            $get_foods_n = explode(",",$diet_data[0]->foods_n);
                            foreach ($get_foods_m as $key => $value) {
                                $get_key = $key;
                               ?>
                            <div class='row'>
                                <div class='col-md-2'>
                                    <label>Day - <?php echo $key + 1; ?></label>
                                    <input type='text' class='form-control' name='day[]' readonly value='Day - <?php echo $key + 1; ?>'>
                                </div>
                                <div class="col-md-3"><label>Morning</label><input type="text" class="form-control" name="foods_m[]" <?php if(!empty($get_foods_m[$get_key])){ ?>value="<?php echo $get_foods_m[$get_key]; ?>" <?php }?>></div>
                                <div class="col-md-2"><label>Afternoon</label><input type="text" class="form-control" name="foods_an[]" <?php if(!empty($get_foods_an[$get_key])){ ?> value="<?php echo $get_foods_an[$get_key]; ?>" <?php }?>></div>
                                <div class="col-md-2"><label>Evening</label><input type="text" class="form-control" name="foods_e[]"  <?php if(!empty($get_foods_e[$get_key])){ ?>  value="<?php echo $get_foods_e[$get_key]; ?>"  <?php }?>></div>
                                <div class="col-md-3"><label>Night</label><input type="text" class="form-control" name="foods_n[]" <?php if(!empty($get_foods_n[$get_key])){ ?> value="<?php echo $get_foods_n[$get_key]; ?>" <?php }?>></div>
                            </div><br>
                            <?php } } ?>
                            
                            <span class="badge badge-info add_row">Add Row</span>
                            <span class="badge badge-info count_row"><?php if(!empty($get_foods_m)){ echo count($get_foods_m); }else{ echo"0"; } ?></span>
                            <div class="row">
                                <div class="col-md-12">
                                    <center>
                                        <input type="hidden" name="diet_id" <?php if(!empty($diet_data)) {?>value="<?php echo $diet_data[0]->diet_id; ?>" <?php } ?> >
...
<script type="text/javascript">
    $(document).ready(function() {
        $('.add_row').click(function(){
            var count_row = parseInt($('.count_row').text());
            $('.count_row').text(++count_row);
            $('.add_row').before("<div class='row'><div class='col-md-2'><label>Day - "+count_row+"</label><input type='text' class='form-control' name='day[]' readonly value='Day - "+count_row+"'></div><div class='col-md-3'><label>Morning</label><input type='text' class='form-control' name='foods_m[]'></div><div class='col-md-2'><label>Afternoon</label><input type='text' class='form-control' name='foods_an[]'></div><div class='col-md-2'><label>Evening</label><input type='text' class='form-control' name='foods_e[]'></div><div class='col-md-3'><label>Night</label><input type='text' class='form-control' name='foods_n[]'></div></div><br>");
        });
